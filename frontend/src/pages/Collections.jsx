...
export default function Collections({ isMobile }) {
  ...
  const handleMarkPaid = async (paymentId) => {
    try {
      ...
      setTimeout(() => { loadPayments(selectedContract.id); load() }, 500)
        ...
        {selectedContract && (
          <div style={CARD}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
              ...
              <button onClick={() => { setSelectedContract(null); localStorage.removeItem('lastContractId') }} style={{ background: 'none', border: 'none', color: '#555', cursor: 'pointer', fontSize: '18px' }}>✕</button>
            </div>
            ...
            <div style={{ maxHeight: '500px', overflowY: 'auto' }}>
              {payments.length === 0 ? (
                <p style={{ color: '#555', textAlign: 'center', padding: '20px' }}>No payments found.</p>
              ) : (
                payments.map((p, i) => (
