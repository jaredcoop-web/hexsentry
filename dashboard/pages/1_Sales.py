...
st.caption(f"{user['business_name']} — Monthly trends, gross profit, and deal volume")
...
try:
    kpis = q(f"SELECT COUNT(*) as total_sales, ROUND(SUM(gross_profit),0) as total_gross, ROUND(AVG(gross_profit),0) as avg_gross FROM {ct('sales')}").iloc[0]
    ...
    monthly = q(f"SELECT month, COUNT(*) as units, ROUND(SUM(gross_profit),0) as gross FROM {ct('sales')} GROUP BY month ORDER BY month")
    ...
    sp = q(f"SELECT salesperson, COUNT(*) as deals, ROUND(SUM(gross_profit),0) as gross, ROUND(AVG(gross_profit),0) as avg_gross FROM {ct('sales')} GROUP BY salesperson ORDER BY gross DESC")
    ...
    source = q(f"SELECT lead_source, COUNT(*) as deals, ROUND(SUM(gross_profit),0) as gross FROM {ct('sales')} GROUP BY lead_source ORDER BY deals DESC")
    st.bar_chart(source.set_index("lead_source")["deals"])

    st.divider()

    st.subheader("Best Selling Models")
    models = q(f"SELECT model, COUNT(*) as units, ROUND(AVG(gross_profit),0) as avg_gross FROM {ct('sales')} GROUP BY model ORDER BY units DESC")
    models["avg_gross"] = models["avg_gross"].apply(lambda x: f"${int(x):,}")
    st.dataframe(models, use_container_width=True, hide_index=True)

except Exception as e:
    st.warning(f"No sales data yet. Upload your sales report on the Upload Data page. ({e})")
