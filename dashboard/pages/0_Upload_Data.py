...
def save_to_db(df, table_name, date_col="date"):
    ...
    try:
        ...
        overlap = set(df[date_col].dt.date) & existing_dates
        if overlap:
...
st.caption(f"{user['business_name']} — Upload your CSV exports and we handle the rest")
...
if uploaded_file:
    try:
        ...
        st.caption("Match your column names to each field. Select 'Skip' if you don't have that field.")
        ...
        if data_type == "Sales Report":
            ...
            if st.button("Process & Save Sales Data", type="primary"):
                try:
                    ...
                    if saved:
                        st.success(f"Saved {saved} new sales records!")
                        st.balloons()
